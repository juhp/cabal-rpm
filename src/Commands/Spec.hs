...
createSpecFile :: Verbosity -> Flags -> Bool -> PackageType -> Bool
               -> Maybe Stream -> Maybe FilePath -> Maybe PackageIdentifier
               -> IO FilePath
createSpecFile verbose flags force pkgtype subpackage mstream mdest mpkgid = do
  ...
  put $ "# generated by cabal-rpm-" ++ V.showVersion Paths_cabal_rpm.version
    +-+ unwords (["--standalone" | standalone] ++ ["--stream " ++ showStream (fromJust mstream) | isJust mstream] ++ ["--subpackage" | subpackage])
  ...
  subpkgs <- if subpackage then
    mapM (getsubpkgMacro mstream specFile >=>
...
createSpecFile_ :: Maybe FilePath -> Verbosity -> Flags -> Bool -> PackageType
                -> Bool -> Maybe Stream -> Maybe PackageIdentifier -> IO ()
createSpecFile_ mdest verbose flags force pkgtype subpackage mstream mpkgid =
  void (createSpecFile verbose flags force pkgtype subpackage mstream mdest mpkgid)
