...
createSpecFile verbose flags force pkgtype subpackage mdest mpvs = do
  ...
  mstream <- case mpvs of
               Nothing -> if specAlreadyExists then
                 withSpecHead specFile $ \ headerwords -> return $ read <$> headerOption headerwords "--stream"
                 else return Nothing
               Just pvs -> return $ pvsStream pvs
...
createSpecFile_ mdest verbose flags force pkgtype subpackage mpvs =
  void (createSpecFile verbose flags force pkgtype subpackage mdest mpvs)
